...
global n_c n_grid del_grid eps sig_amp n_d_min n_d_in n_d_max iter_max...
       n_steps sig_e recomb_L a_crack D Length_unit G nu limit E n_branch ...
       tough_limit c_scale alpha q resistance C_T C_L lambda xbound ybound rho Eeff K_IC
...
n_d_in = 2;
n_d_min=7;            % minimum number of dipoles on crack branches
n_d_max=10;
eps=1e-3;               % Convergence criterion for PK forces
n_steps=4;              % number of steps for a dislocation to reach its neighbor, the smaller the faster they move
iter_max=3000;           % max iterations for convergence= iter_max*n_d_min
...
% crack_size=6e-3;         %%%%% initial crack size (2a=12 mm)
% % P=1e3;                  % disk load [N]
...
crack_size = 6e-2;          % Initial crack size (2a = 6 cm)
%%%%%%%%%%%%%%%%%%%  GRID AND UNITS   %%%%%%%%%%%%%%%%%%%%%%%%%%
...
sig_e = [0,0,0;0,1.151e-3,0;0,0,0];            % External stress tensor for simple tension (Normalized with shear modules G)
