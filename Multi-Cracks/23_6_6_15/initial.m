...
global  n_c n_grid del_grid C  ...
        S_xx S_yy S_xy Sigma_xx Sigma_yy Sigma_xy X Y  force_ratio ...
         e_x e_y e_z t position u_x u_y U V n_d_min x_cod_up ....
        y_cod_up x_cod_down y_cod_down sig_amp sig_e
...
for i=1:n_c
        ...
        C(i).theta=atan2(C(i).trac(2),C(i).trac(1));
    for j=1:2*C(i).n_d     
        C(i).D(j).R_p=C(i).R_L+C(i).d*(j-1)*C(i).e_p;
        C(i).D(j).F_PK=ones(3);
%         C(i).D(j).b=-C(i).b_mag*C(i).e_p;
